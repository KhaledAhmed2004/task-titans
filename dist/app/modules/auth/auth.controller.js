...
const googleCallback = (0, catchAsync_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!user) {
            ...
            return res.redirect(`https://task-titans-six.vercel.app/error?message=Google authentication failed. No user data received.`);
        }
        ...
        return res.redirect(`https://task-titans-six.vercel.app/success?token=${result.createToken}`);
    }
    catch (error) {
        console.error('💥 Google OAuth callback error:', error);
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        return res.redirect(`https://task-titans-six.vercel.app/error?message=${encodeURIComponent(errorMessage)}`);
    }
}));
