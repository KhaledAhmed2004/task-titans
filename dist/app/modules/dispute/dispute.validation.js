"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.DisputeValidation = void 0;
const zod_1 = require("zod");
const dispute_interface_1 = require("./dispute.interface");
const createDisputeSchema = zod_1.z.object({
    body: zod_1.z.object({
        taskId: zod_1.z
            .string({
            required_error: 'Task ID is required',
        })
            .regex(/^[0-9a-fA-F]{24}$/, 'Invalid task ID format'),
        type: zod_1.z.nativeEnum(dispute_interface_1.DisputeType, {
            required_error: 'Dispute type is required',
        }),
        title: zod_1.z
            .string({
            required_error: 'Dispute title is required',
        })
            .min(5, 'Title must be at least 5 characters long')
            .max(200, 'Title cannot exceed 200 characters'),
        description: zod_1.z
            .string({
            required_error: 'Dispute description is required',
        })
            .min(20, 'Description must be at least 20 characters long')
            .max(2000, 'Description cannot exceed 2000 characters'),
        posterId: zod_1.z
            .string()
            .regex(/^[0-9a-fA-F]{24}$/, 'Invalid poster ID format')
            .optional(),
        freelancerId: zod_1.z
            .string()
            .regex(/^[0-9a-fA-F]{24}$/, 'Invalid freelancer ID format')
            .optional(),
        evidence: zod_1.z
            .array(zod_1.z.object({
            type: zod_1.z
                .enum(['screenshot', 'document', 'communication', 'other'])
                .optional(),
            description: zod_1.z.string().min(10).max(500).optional(),
            attachments: zod_1.z.array(zod_1.z.string().url()).optional(),
        }))
            .optional(),
    }),
});
const addEvidenceSchema = zod_1.z.object({
    params: zod_1.z.object({
        disputeId: zod_1.z
            .string({
            required_error: 'Dispute ID is required',
        })
            .regex(/^[0-9a-fA-F]{24}$/, 'Invalid dispute ID format'),
    }),
    body: zod_1.z.object({
        type: zod_1.z.enum(['screenshot', 'document', 'communication', 'other'], {
            required_error: 'Evidence type is required',
        }),
        description: zod_1.z
            .string({
            required_error: 'Evidence description is required',
        })
            .min(10, 'Description must be at least 10 characters long')
            .max(500, 'Description cannot exceed 500 characters'),
        attachments: zod_1.z.array(zod_1.z.string().url()).optional(),
    }),
});
const updateDisputeStatusSchema = zod_1.z.object({
    params: zod_1.z.object({
        disputeId: zod_1.z
            .string({
            required_error: 'Dispute ID is required',
        })
            .regex(/^[0-9a-fA-F]{24}$/, 'Invalid dispute ID format'),
    }),
    body: zod_1.z.object({
        status: zod_1.z.nativeEnum(dispute_interface_1.DisputeStatus, {
            required_error: 'Status is required',
        }),
    }),
});
const resolveDisputeSchema = zod_1.z.object({
    params: zod_1.z.object({
        disputeId: zod_1.z
            .string({
            required_error: 'Dispute ID is required',
        })
            .regex(/^[0-9a-fA-F]{24}$/, 'Invalid dispute ID format'),
    }),
    body: zod_1.z.object({
        resolution: zod_1.z.nativeEnum(dispute_interface_1.DisputeResolution, {
            required_error: 'Resolution decision is required',
        }),
        adminNotes: zod_1.z.string().min(10).max(1000).optional(),
        refundPercentage: zod_1.z.number().min(0).max(100).optional(),
    }),
});
// ------------------------
// Get Disputes Validation (Query Filters & Pagination)
// ------------------------
const getDisputesSchema = zod_1.z.object({
    query: zod_1.z.object({
        status: zod_1.z.nativeEnum(dispute_interface_1.DisputeStatus).optional(),
        type: zod_1.z.nativeEnum(dispute_interface_1.DisputeType).optional(),
        page: zod_1.z
            .string()
            .transform(val => parseInt(val, 10))
            .optional(),
        limit: zod_1.z
            .string()
            .transform(val => parseInt(val, 10))
            .optional(),
    }),
});
exports.DisputeValidation = {
    createDisputeSchema,
    addEvidenceSchema,
    updateDisputeStatusSchema,
    resolveDisputeSchema,
    getDisputesSchema,
};
