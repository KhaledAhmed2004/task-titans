...
const getReportStats = () => __awaiter(void 0, void 0, void 0, function* () {
    const totalReports = yield report_model_1.Report.countDocuments();
    const totalPending = yield report_model_1.Report.countDocuments({
        status: report_interface_1.REPORT_STATUS.PENDING,
    });
    const totalReviewed = yield report_model_1.Report.countDocuments({
        status: report_interface_1.REPORT_STATUS.REVIEWED,
    });
    const totalResolved = yield report_model_1.Report.countDocuments({
        status: report_interface_1.REPORT_STATUS.RESOLVED,
    });
    const calculateMonthlyGrowth = (...args_1) => __awaiter(void 0, [...args_1], void 0, function* (filter = {}) {
        ...
        const startOfLastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
        const endOfLastMonth = new Date(now.getFullYear(), now.getMonth(), 0);
        const thisMonthCount = yield report_model_1.Report.countDocuments(Object.assign(Object.assign({}, filter), { createdAt: { $gte: startOfThisMonth } }));
        const lastMonthCount = yield report_model_1.Report.countDocuments(Object.assign(Object.assign({}, filter), { createdAt: { $gte: startOfLastMonth, $lte: endOfLastMonth } }));
        let monthlyGrowth = 0;
    ...
    const allReportStats = yield calculateMonthlyGrowth();
    const pendingStats = yield calculateMonthlyGrowth({
        status: report_interface_1.REPORT_STATUS.PENDING,
    });
    const reviewedStats = yield calculateMonthlyGrowth({
        status: report_interface_1.REPORT_STATUS.REVIEWED,
    });
    const resolvedStats = yield calculateMonthlyGrowth({
        status: report_interface_1.REPORT_STATUS.RESOLVED,
    });
    return {
        allReports: Object.assign({ total: totalReports }, allReportStats),
        pending: Object.assign({ total: totalPending }, pendingStats),
        reviewed: Object.assign({ total: totalReviewed }, reviewedStats),
        resolved: Object.assign({ total: totalResolved }, resolvedStats),
    };
});
