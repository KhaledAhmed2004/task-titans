...
const googleCallback = catchAsync(async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      ...
      return res.redirect(
        `https://task-titans-six.vercel.app/callback?message=Google authentication failed. No user data received.`
      );
    }
    ...
    return res.redirect(
      `https://task-titans-six.vercel.app/callback?token=${result.createToken}`
    );
  } catch (error) {
    ...
    return res.redirect(
      `https://task-titans-six.vercel.app/callback?message=${encodeURIComponent(
        errorMessage
      )}`
    );
  }
